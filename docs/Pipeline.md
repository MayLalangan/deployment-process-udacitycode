...
graph TD
    ...
    GitHub -- Webhook --> CircleCI[CircleCI Pipeline]
    
    subgraph CircleCI Workflow
        Build[Build Job<br/>Install & Compile] --> Hold{Hold<br/>Manual Approval}
        Hold -- Approved --> Deploy[Deploy Job]
    end
    
    Deploy -- Upload Code --> AWS_S3[AWS S3<br/>Frontend]
    Deploy -- Upload Code --> AWS_EB[AWS Elastic Beanstalk<br/>Backend]
```
